...
namespace BootcampAres.Application.Services
{
    public class CustomerService : ICustomerService
    {
        ...
        public CustomerResponse? GetCustomerByNumber(int number)
        {
            ...
            if (customer != null)
            {
                CustomerResponse result = CustomerMapper.MapToCustomerResponseFromCustomerDto(customer);
                return result;
            }
            else
                return null;
        ...
        public CustomerWithEmployeeResponseList CustomerSearch(CustomerSearchRequest request)
        {
            if (request.PageNumber < 1)
                return new CustomerWithEmployeeResponseList { Error = "El numero de pagina debe ser igual a 1 o superior." };

            if (request.ItemsPerPage <= 0)
                return new CustomerWithEmployeeResponseList { Error = "El numero de elementos por pagina debe ser mayor que 0." };

            CustomerWithEmployeeDtoList searchList = _customerRepository.CustomerSearch(request.PageNumber, request.ItemsPerPage, request.CustomerName, request.ContactFirstName, request.City, request.Country, request.EmployeeFirstName);

            CustomerWithEmployeeResponseList result = CustomerMapper.MapToCWithEResponseListFromCWithEDtoList(searchList, request.PageNumber, request.ItemsPerPage);
